...
const App: React.FC = () => {
    ...
    useEffect(() => {
        if ('mediaSession' in navigator && currentTrack) {
            ...
            // Sync playbackState synchronously for OS media notification UI
            ...
            const setHandler = (action: MediaSessionAction, handler: MediaSessionActionHandler | null) => {
                try {
                    ...
                    // Handler not supported on current platform/browser version
                }
            };
            ...
            setHandler('play', () => {
                try {
                    ...
                    console.error('MediaSession play action error:', e);
                }
            });
            ...
            setHandler('pause', () => {
                try {
                    ...
                    console.error('MediaSession pause action error:', e);
                }
            });
            ...
            setHandler('stop', () => {
                try {
                    ...
                    console.error('MediaSession stop action error:', e);
                }
            });
            ...
            setHandler('seekbackward', (details) => {
                ...
                seekTo(Math.max((currentTime || 0) - offset, 0));
            });

            setHandler('seekforward', (details) => {
                const offset = details.seekOffset || 10;
                seekTo(Math.min((currentTime || 0) + offset, duration || 0));
            });

            // Update position state safely
            if ('setPositionState' in navigator.mediaSession && duration > 0 && !isNaN(duration) && !isNaN(currentTime)) {
                try {
                    const validPosition = Math.min(Math.max(currentTime, 0), duration);
                    ...
                    // Ignore invalid state parameter errors
                }
            }
        }
    }, [currentTrack, isPlaying, playNext, playPrev, currentTime, duration, seekTo, play, pause]);
